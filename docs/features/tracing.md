...
| `OTEL_SERVICE_NAME`           | `app-api` (from `APP_NAME`) | Value of the `service.name` resource attribute on every span; also the `service` field on log lines. Parsed in `env.ts`.                                                                                                                                                                                                                   |
