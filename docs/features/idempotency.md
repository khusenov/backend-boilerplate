...
| Container wiring                                                                | Composition root   | Registers `idempotencyRedis` (dedicated ioredis singleton with a disconnect disposer) and binds `idempotencyStore` to `RedisIdempotencyStore` with the two env TTLs                                                                             | `src/container.ts`                                          |
...
**Swapping the backend.** Implement `IdempotencyStore` in a new infrastructure adapter and rebind the `idempotencyStore` registration in `src/container.ts`; the plugin and routes are untouched.
