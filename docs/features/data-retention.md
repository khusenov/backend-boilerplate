...
> **Status:** Complete · **Layers:** domain, application, infrastructure · **Verified against:** `329a35a`

## Purpose

Operational tables accumulate rows that stop being useful once they age out: expired refresh tokens that can never authenticate again, and outbox messages — rows recording domain events for reliable delivery — that have already been relayed (published). Left alone they grow without bound, slow queries, and hold personal data longer than a retention policy allows. This feature is a scheduled housekeeping job that periodically prunes those stale rows. It is deliberately built as an **open set of prune tasks** behind a single `RetentionTask` port: the job that runs them knows nothing about tokens or outbox rows, so adding a new prunable resource is a new adapter plus a few lines of container wiring (see Usage & extension), never a change to the scheduler.

## How it works

Two independent quantities drive the feature, and it is worth separating them up front:

- **Cadence** — _how often_ the job runs. Fixed at hourly by the code constant `DATA_RETENTION_INTERVAL_MS` (`60 * 60 * 1000`).
...
1. **The worker starts.** `void app.diContainer.resolve('jobWorker')` resolves the `JobWorker` singleton, which begins consuming the shared `default` BullMQ queue. Its handler list (assembled in `container.ts`) includes `enforceDataRetentionJob`, whose `jobName` is `data.retention`.
2. **The job is scheduled.** `await jobScheduler.schedule(DATA_RETENTION_JOB, {}, { everyMs: DATA_RETENTION_INTERVAL_MS })` registers `data.retention` as a repeatable job. Backed by BullMQ's `upsertJobScheduler`, this is idempotent per job name, so re-running bootstrap re-registers the same schedule rather than stacking duplicates. The payload is an empty object — the handler ignores it.

**From then on, once an hour:**

3. **BullMQ enqueues the job** onto the `default` queue, and `JobWorker.process` routes it by name to `EnforceDataRetentionJob`.
4. **The job computes a single cutoff.** `EnforceDataRetentionJob.handle()` calculates `cutoff = new Date(Date.now() - this.dataRetentionWindowMs)` — the instant `DATA_RETENTION_TTL` seconds ago (30 days ago by default).
5. **Every task is pruned with that cutoff.** The job iterates its `retentionTasks` array — `[refreshTokenRetentionTask, outboxRetentionTask]` — and awaits `task.prune(cutoff)` on each. On success it logs `'Pruned expired records'` with the task's `resource` label, the `deleted` count, and the `cutoff`.
...
- **`RefreshTokenRetentionTask`** delegates to `refreshTokenRepository.deleteExpired(cutoff)`, which runs `deleteMany({ where: { expiresAt: { lt: cutoff } } })` against the `refresh_tokens` table. A token is pruned only once it has been **expired** for the full retention window — not merely created that long ago. With the default 14-day refresh-token TTL and 30-day window, that is roughly 44 days after the token was issued.
- **`OutboxRetentionTask`** runs `prisma.outboxMessage.deleteMany({ where: { publishedAt: { lt: cutoff } } })` against the `outbox_messages` table. Only **relayed (published)** messages are deleted: a row whose `publishedAt` is still `NULL` (not yet relayed — see [domain events](./domain-events.md)) is excluded by the `lt` comparison and is never pruned, so retention can never drop an event that still needs delivery.
...
| `RefreshTokenRepository.deleteExpired` | Domain (interface) / Infrastructure (impl) | Deletes `refresh_tokens` where `expiresAt < cutoff`, returns the count                                                                          | `src/domain/auth/refresh-token-repository.ts`, `src/infrastructure/persistence/prisma-refresh-token-repository.ts` |
...
| `JobScheduler`                         | Application (port)                         | Registers the repeatable `data.retention` job at bootstrap                                                                                      | `src/application/shared/ports/job-scheduler.ts`                                                                    |
| Container wiring                       | Composition root                           | Binds the tasks to `RetentionTask`, sets `dataRetentionWindowMs`, assembles the job's task list, and adds the job to the worker                 | `src/container.ts`                                                                                                 |
| Bootstrap                              | Composition root                           | Schedules `data.retention` on the hourly interval                                                                                               | `src/main.ts`                                                                                                      |
...
- **`resource`** — a stable, log-friendly label for the table this task prunes (the existing tasks use `'refresh_tokens'` and `'outbox_messages'`). `EnforceDataRetentionJob` includes it verbatim in every success and failure log line, so it is how a resource's pruning is identified in the logs.
...
// inside registerDependencies(...), beside refreshTokenRetentionTask / outboxRetentionTask
...
enforceDataRetentionJob: asFunction(
  ...
  >) =>
    new EnforceDataRetentionJob({
      retentionTasks: [refreshTokenRetentionTask, outboxRetentionTask, auditLogRetentionTask],
      dataRetentionWindowMs,
      logger,
    }),
).singleton(),
...
- **Cadence is a constant; the retention window is configuration.** How _often_ the sweep runs (`DATA_RETENTION_INTERVAL_MS`, hourly) rarely needs operational tuning, so it is a code constant. How _long_ data is kept (`DATA_RETENTION_TTL`) is a policy/compliance decision that differs per environment, so it is the one env var. An hourly cadence against a 30-day window means a row lingers at most an extra hour past its window — cheap insurance against a job worker being briefly down.
- **Runs on the shared BullMQ transport, not an in-process timer.** Scheduling goes through `JobScheduler.schedule` (a repeatable BullMQ job), consistent with the project rule that all background and scheduled work is Redis-backed. This gives the sweep durability across restarts and keeps it on the same worker and observability path as every other job, at the cost of the standing Redis dependency the transport already requires.

## Testing

All three units are covered by fast, dependency-free unit tests (Vitest with fakes — no database or Redis). Run them with `npm test` (`vitest run`).

- **`src/application/retention/enforce-data-retention-job.test.ts`** exercises the orchestration with fake `RetentionTask`s and a stub `Logger`: `jobName` equals `DATA_RETENTION_JOB`; every task is pruned with the cutoff derived from _now minus the window_ (asserted against fake timers pinned to a fixed `NOW`); the deleted count is logged per resource via `'Pruned expired records'`; when one task throws it keeps pruning the rest and logs `'Failed to prune expired records'` with the error message; a non-`Error` rejection is stringified in that log; and with no tasks registered it does nothing (no log calls).
...
There is no integration test that stands up a real database or drives the full `schedule → worker → prune` path for this job; the end-to-end job transport it rides on is covered separately (see [background jobs](./background-jobs.md#testing)).
