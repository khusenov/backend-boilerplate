...
3. **Roles and assignments are managed through use cases.** The `/roles` endpoints drive
   ...
   resource (`AssignRole`, `RevokeRole`). Read-only use cases take the narrow `RoleReader` port and
   resolve `PrismaRoleReader` from the container; the three mutations run inside `UnitOfWork.run` and
   get a `PrismaRoleRepository` — the only class with `save` — from the transaction context. Because permissions are re-read
...
The feature obeys the dependency rule. The **domain** owns the `Role` aggregate, the `RoleReader` /
`RoleRepository` _interfaces_, the permission `PERMISSIONS` catalogue, the `Actor` union and the `ensurePermission`
access policy, and the typed domain errors — it depends on nothing outside `domain`. The
**application** layer holds the use cases and the ports they need (`RoleReader` / `RoleRepository`
from domain, plus
...
| `RoleReader` / `RoleRepository`                                                                                                              | Domain             | Ports: `RoleReader` lists/finds; `RoleRepository` extends it with the guarded `save`          | `src/domain/authorization/role-repository.ts`                                           |
...
| `PrismaRoleReader`                                                                                                                           | Infrastructure     | `RoleReader` adapter: the four reads, no write path                                           | `src/infrastructure/persistence/prisma-role-reader.ts`                                  |
| `PrismaRoleRepository` / `toDomain` / `toPersistence`                                                                                        | Infrastructure     | Extends the reader with a version-guarded `save` (diffs `rolePermission` join rows) + mappers | `src/infrastructure/persistence/prisma-role-repository.ts`, `.../prisma-role-mapper.ts` |
