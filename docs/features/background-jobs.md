...
| --------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------ |
| `GET` (and Bull Board's own asset/API routes) | all routes under `BULL_BOARD_PATH` (default `/admin/queues`) | HTTP Basic (`Authorization: Basic …`), realm `app Queues`; only mounted when `BULL_BOARD_ENABLED=true` | Browse and (unless `BULL_BOARD_READONLY`) act on queued jobs |
...
// in the module's Cradle slice, near exampleJobHandler — the port, not the class
...
- **Two separate Redis connections, one for producing and one for the worker.** `src/composition/jobs.ts` registers `redisConnection` (used by `BullMqJobQueue`, `BullMqJobScheduler`, and the dashboard queue) and `workerConnection` (used by `JobWorker`) as distinct ioredis singletons. A BullMQ worker holds a _blocking_ connection while it waits for jobs; sharing that socket with the producer side would stall enqueues. Both are created via `createRedisConnection` with `maxRetriesPerRequest: null`, which BullMQ requires so its blocking commands are never aborted mid-wait.
