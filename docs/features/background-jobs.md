...
- **The dead-letter signal is a log line, not a metric.** A job BullMQ declines to retry is logged once at `error` as `'Job dead-lettered, no retry remains'`, carrying the job name and id — and because the outbox relay uses the outbox row id as its deduplication key, that job id traces a dead-lettered event back to the exact database row that produced it. It is also retained in the failed set for 7 days and inspectable in Bull Board when `BULL_BOARD_ENABLED=true` (it defaults to `false`, so until it is enabled the log line is the only signal). Alloy already forwards the log to Loki, so the signal is observable today. The worker process now exposes its own `/metrics` endpoint (mounted on `buildHealthApp` alongside `/health`, gated by the same `METRICS_ENABLED` flag as the HTTP app), so a registry now has somewhere to be scraped from — but no code yet increments a dead-letter counter. `MetricsRecorder` would need a new method (e.g. `recordJobDeadLettered`) called from the worker's failure handler before a counter can replace the log line as the primary signal.
