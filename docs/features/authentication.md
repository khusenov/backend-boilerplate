...
| `WEB_ORIGIN`          | `—` (devDefault `http://127.0.0.1:3000`) | CORS origin; the app enables `credentials: true` so the browser sends the cookie cross-origin. Declared with `devDefault`, so it has **no** production default — it is required at boot when `NODE_ENV=production`. Owned by [http-infrastructure.md](./http-infrastructure.md).                                                                             |
