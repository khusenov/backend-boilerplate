...
**The Dependency Rule:** dependencies point **inward**. The domain depends on nothing; the application depends on the domain plus its own ports; infrastructure and presentation depend inward through those abstractions. Concrete implementations are bound to ports **only** in `src/composition/**`; `src/container.ts` builds the container and hands it the merged registration map.
...
| Feature                    | Doc                                                                   | Summary                                                                                                                                                                                                                                    |
| -------------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| HTTP Infrastructure        | [http-infrastructure.md](./features/http-infrastructure.md)           | The shared Fastify application factory: `buildApp` fixes cross-cutting plugin order, mounts the versioned `/v1` surface, and defines request validation, response shaping, and the uniform JSON error envelope every HTTP feature sits on. |
