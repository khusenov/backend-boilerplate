...
describe('/auth/verify-email (integration)', () => {
  ...
  it('enqueues a six-digit code that is not what the database stores', async () => {
    const { id, code } = await register('shape@finflow.test');
  ...
  it('activates the account and consumes the code (200)', async () => {
    const { id, code } = await register('verify@finflow.test');

    const res = await verify('verify@finflow.test', code);
  ...
  it('rejects a replay of an already-consumed code (400)', async () => {
    const { code } = await register('replay@finflow.test');
    expect((await verify('replay@finflow.test', code)).statusCode).toBe(200);

    const replay = await verify('replay@finflow.test', code);
  ...
  it('rejects a wrong code and records the attempt (400)', async () => {
    const { id, code } = await register('wrong@finflow.test');

    const res = await verify('wrong@finflow.test', anyCodeOtherThan(code));
  ...
  it('locks the code once the attempt cap is reached (400)', async () => {
    const { id, code } = await register('capped@finflow.test');
    const wrong = anyCodeOtherThan(code);

    for (let i = 0; i < env.VERIFICATION_MAX_ATTEMPTS; i++) {
      const res = await verify('capped@finflow.test', wrong);
      expect(res.json<ErrorBody>().error.code).toBe('VERIFICATION_CODE_INVALID');
    }

    const capped = await verify('capped@finflow.test', wrong);
  ...
  it('refuses even the correct code after the cap, leaving the user pending', async () => {
    const { id, code } = await register('burned@finflow.test');
    const wrong = anyCodeOtherThan(code);

    for (let i = 0; i < env.VERIFICATION_MAX_ATTEMPTS; i++) {
      await verify('burned@finflow.test', wrong);
    }

    const res = await verify('burned@finflow.test', code);
  ...
  it('answers an unknown email exactly as it answers a wrong code (400)', async () => {
    const { code } = await register('known@finflow.test');

    const unknown = await verify('nobody@finflow.test', '123456');
    const wrong = await verify('known@finflow.test', anyCodeOtherThan(code));
  ...
  it.each(['12345', '1234567', 'abcdef', ''])('rejects a malformed code %j (400)', async (code) => {
    const res = await verify('someone@finflow.test', code);
  ...
  it('register -> login 401 -> verify -> login 200 -> authorized endpoint 403', async () => {
    const { code } = await register('flow@finflow.test');

    const before = await h.app.inject({
      method: 'POST',
      url: '/v1/auth/login',
      payload: { email: 'flow@finflow.test', password: PASSWORD },
    });
    expect(before.statusCode).toBe(401);

    expect((await verify('flow@finflow.test', code)).statusCode).toBe(200);
