...
export const env = cleanEnv(process.env, {
  ...
  TRUST_PROXY: str({
    default: 'false',
    desc: 'false when exposed directly; the number of proxy hops to trust when behind an ingress',
  }),
  BODY_LIMIT_BYTES: num({ default: 1024 * 1024 }),
  REQUEST_TIMEOUT_MS: num({ default: 30_000 }),
  KEEP_ALIVE_TIMEOUT_MS: num({ default: 72_000 }),
  MAX_PARAM_LENGTH: num({ default: 100 }),
